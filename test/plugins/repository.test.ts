...
import { IAuth } from '../../src/schemas/auth.js';
...
test("repository works standalone", async (t) => {
  ...
  // Test find method
  const user = await app.repository.find<IAuth>('users', { select: 'username', where: { username: 'basic' } });
  assert.deepStrictEqual(user, { username: 'basic' });

  const firstUser = await app.repository.find<IAuth>('users', { select: 'username' });
  assert.deepStrictEqual(firstUser, { username: 'basic' });

  const nullUser  = await app.repository.find<IAuth>('users', { select: 'username', where: { username: 'unknown' } });
  assert.equal(nullUser, null);

  // Test findMany method
  const users = await app.repository.findMany<IAuth>('users', { select: 'username', where: { username: 'basic' } });
  assert.deepStrictEqual(users, [
    { username: 'basic' }
  ]);

  // Test findMany method
  const allUsers = await app.repository.findMany<IAuth>('users', { select: 'username' });
  ...
  const newUser = await app.repository.find<IAuth>('users', { select: 'username', where: { id: newUserId } });
  ...
  const updatedUser = await app.repository.find<IAuth>('users', { select: 'password', where: { username: 'new_user' } });
  ...
  const deletedUser = await app.repository.find<IAuth>('users', { select: 'username', where: { username: 'new_user' } });
